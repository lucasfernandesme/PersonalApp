    ...
    <div className="min-h-screen flex flex-col bg-slate-50 text-slate-900 pb-20 md:pb-0">
      ...
      <aside className="w-64 bg-slate-900 text-white flex-shrink-0 flex-col hidden md:flex h-screen sticky top-0">
        ...
        <div className="px-6 pb-2 -mt-4 mb-2">
          <p className="text-[10px] uppercase font-bold text-slate-500">Frequência de treinos</p>
          <p className="text-xs font-bold text-white">Segunda a Domingo</p>
        </div>
...
const NavItem = ({ icon, label, active = false, onClick }: { icon: React.ReactNode, label: string, active?: boolean, onClick?: () => void }) => (
  <button onClick={onClick} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${active ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/20' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
    }`}>
    {icon}
