...
const Layout: React.FC<LayoutProps> = ({ children, role, onSwitchRole, onNavigate, activeTab, userName, userAvatar, trainerSocials }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-zinc-950 text-slate-900 dark:text-zinc-100 pb-20 md:pb-0 transition-colors duration-300">
      ...
      {/* Mobile Top Header */}
      <header className="md:hidden bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between px-4 sticky top-0 z-40 transition-all duration-300 pt-[calc(1rem+env(safe-area-inset-top))] pb-4 relative">
        <button onClick={toggleTheme} className="text-slate-400 p-2 z-10 w-10">
        ...
        <button onClick={onSwitchRole} className="text-slate-400 p-2 z-10 w-10 flex justify-end">
          <LogoutIcon size={20} />
        </button>
      </header>
