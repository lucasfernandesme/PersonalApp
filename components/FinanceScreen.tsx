...
const FinanceScreen: React.FC<FinanceScreenProps> = ({ user, onBack }) => {
    ...
    const handleTogglePayment = async (student: any) => {
        const existingPayment = data.payments.find(p => p.studentId === student.id && p.type === 'revenue' && p.category === 'Mensalidade');
    ...
    const revenues = data.payments.filter(p => p.type === 'revenue');
    ...
    const filteredPayments = data.payments.filter(item => {
        if (activeTab === 'revenues') return item.type === 'revenue';
    ...
    const groupedPayments = filteredPayments
        ...
        .reduce((acc, p) => {
            ...
            acc[cat].total += p.type === 'revenue' ? (p.amount || 0) : -(p.amount || 0);
            return acc;
