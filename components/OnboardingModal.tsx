...
const OnboardingModal: React.FC<OnboardingModalProps> = ({ students, onClose, onProceedToManual, onAISuccess }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-white dark:bg-slate-900 flex flex-col md:inset-4 md:rounded-[40px] md:shadow-2xl md:max-w-md md:mx-auto md:overflow-hidden animate-in slide-in-from-bottom duration-300 transition-colors">
      {/* Header */}
      <div className="p-6 flex items-center justify-between border-b border-slate-100 dark:border-slate-800 transition-colors">
        <button onClick={step === 1 ? onClose : () => setStep(s => s - 1)} className="p-2 -ml-2 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
          <ChevronLeft size={24} />
        </button>
        ...
        <button onClick={onClose} className="p-2 text-slate-300 dark:text-slate-600 hover:text-slate-400 dark:hover:text-slate-400"><X size={20} /></button>
      </div>
