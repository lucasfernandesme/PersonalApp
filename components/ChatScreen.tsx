  ...
  return (
    <div className="flex flex-col h-[calc(100vh-140px)] md:h-[calc(100vh-120px)] animate-in fade-in duration-500">
      ...
      <div className="flex items-center gap-3 mb-6 px-1">
        ...
        <div>
          <h2 className="text-xl font-black text-slate-900 leading-none">FitAI Assistant</h2>
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Inteligência Artificial Ativa</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-4 pr-2 no-scrollbar">
        ...
        {isLoading && (
          <div className="flex justify-start animate-pulse">
            <div className="bg-white border border-slate-100 p-4 rounded-[24px] rounded-tl-none flex items-center gap-2">
              <Loader2 size={16} className="animate-spin text-indigo-600" />
              <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">IA pensando...</span>
            </div>
          </div>
        )}
      ...
      <div className="mt-4 pb-4 md:pb-0">
        <div className="relative group">
          <input 
            ...
            className="w-full bg-white border-2 border-slate-100 rounded-[28px] pl-6 pr-16 py-5 font-bold text-slate-700 focus:border-indigo-500 focus:ring-0 transition-all shadow-xl shadow-slate-200/50"
          />
          <button 
            onClick={handleSend}
            disabled={!input.trim() || isLoading}
            className={`absolute right-2 top-2 w-12 h-12 rounded-full flex items-center justify-center transition-all ${
              input.trim() ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-100 text-slate-300'
            }`}
          >
        ...
        <div className="flex items-center justify-center gap-2 mt-3">
          ...
          <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Powered by Gemini 3 Flash</span>
        </div>
      </div>
    </div>
  );
