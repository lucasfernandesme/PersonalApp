...
const EvolutionScreen: React.FC<EvolutionScreenProps> = ({ students, onSelectStudent }) => {
  const getStatus = (history: any[]) => {
    if (history.length === 0) return { label: 'Sem dados', color: 'bg-slate-100 text-slate-400' };
    ...
    if (avgComp < 70) return { label: 'Em Alerta', color: 'bg-amber-50 text-amber-700 border-amber-100' };
    if (avgRpe < 7) return { label: 'Aumentar Carga', color: 'bg-indigo-50 text-indigo-700 border-indigo-100' };
    return { label: 'Consistente', color: 'bg-emerald-50 text-emerald-700 border-emerald-100' };
  };

  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col gap-2">
        <h2 className="text-3xl font-black text-slate-900 tracking-tight">Evolução Global</h2>
        <p className="text-slate-400 font-medium">Acompanhe a performance de todos os seus alunos.</p>
      </div>
        ...
        {students.map(student => {
          ...
          return (
            <div 
              ...
              className="bg-white rounded-[32px] border border-slate-200 p-6 hover:shadow-xl transition-all cursor-pointer group"
            >
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="grid grid-cols-3 gap-8 flex-1 md:max-w-md">
                  <div className="space-y-1 text-center md:text-left">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1 justify-center md:justify-start">
                      <BarChart3 size={12} /> RPE Médio
                    </p>
                    <p className="text-lg font-black text-slate-800">{avgRpe}</p>
                  </div>
                  <div className="space-y-1 text-center md:text-left">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1 justify-center md:justify-start">
                      <Zap size={12} /> Frequência
                    </p>
                    <p className="text-lg font-black text-slate-800">{student.history.length} <span className="text-xs text-slate-300">treinos</span></p>
                  </div>
                  <div className="space-y-1 text-center md:text-left">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1 justify-center md:justify-start">
                ...
                <div className="hidden md:flex w-10 h-10 bg-slate-50 rounded-xl items-center justify-center text-slate-300 group-hover:bg-indigo-600 group-hover:text-white transition-all">
      ...
      {students.length === 0 && (
        <div className="py-20 text-center bg-white rounded-[40px] border border-dashed border-slate-200">
          ...
          <p className="text-slate-400 font-bold">Cadastre alunos para ver a evolução aqui.</p>
        </div>
      )}
    </div>
  );
