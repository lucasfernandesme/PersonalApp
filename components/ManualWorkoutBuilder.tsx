...
const ManualWorkoutBuilder: React.FC<ManualWorkoutBuilderProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[70] bg-zinc-50 dark:bg-zinc-950 flex flex-col animate-in slide-in-from-right duration-300">
      <header className="bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 p-4 pt-14 md:pt-4 sticky top-0 z-[100] flex items-center justify-between shadow-sm">
        <button onClick={onCancel} className="relative p-3 -ml-2 text-zinc-400 dark:text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300 transition-colors active:bg-zinc-100 dark:active:bg-zinc-800 rounded-full z-50">
          <ArrowLeft size={28} />
        </button>
        <div className="text-center">
          ...
          <p className="text-[10px] font-bold text-zinc-400 dark:text-zinc-500">{isTemplateMode ? 'Biblioteca de Treinos' : studentName}</p>
        </div>
        <button
          onClick={handleSave}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 pb-24 scrollbar-thin scrollbar-thumb-zinc-200 dark:scrollbar-thumb-zinc-800">
        {error && (
